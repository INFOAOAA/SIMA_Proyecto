...
for anio in range(2020, 2026):
    nombre_excel = os.path.abspath(f'BasesDeDatosXlsx/BD {anio}.xlsxs')
    nombre_csv_salida = os.path.abspath(f'BasesDeDatosCsv/BD_{anio}_Unido.csv')
...
path = os.path.abspath('BasesDeDatosCsv/BD_2022_Unido.csv')
...
df_clean['Fecha y hora'] = pd.to_datetime(df_clean['Fecha y hora'], yearfirst=True)
bins_seasons = ['']
...
```{python}
os.path.abspath('BasesDeDatosCsv/Limpia/BD_2022_Limpia.csv')
...
exp_year = pd.read_csv('BasesDeDatosCsv/Limpia/BD_2022_Limpia.csv')
data = exp_year
