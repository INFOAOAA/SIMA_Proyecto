...
def kmo(df: pd.DataFrame, threshold: float = 0.6) -> list[str]:
    ...
    while not_yet:
        ...
        kmo_series = pd.Series(
            ...
            ascending=True
        )  # Sort ascending to identify lowest easily
...
def factorial_analysis(df: pd.DataFrame, n_factors: int = 3, rotation: str = "varimax"):
...
def main():
    ...
    scree_plot(df=df)
    factorial_analysis(
        df=df.select_dtypes(include=[np.number, bool]).dropna(), n_factors=3
    )
