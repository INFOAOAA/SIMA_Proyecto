...
* **Prueba de Ljung-Box (Ausencia de Autocorrelación):** Se evaluó la hipótesis nula de independencia de residuos hasta el retardo $k = 48$. Para los tres modelos, los p-valores superaron el umbral de significancia ($p > 0.05$), confirmando que la estructura autorregresiva y estacional ($s=24$) absorbió la dependencia temporal de las series.
* **Homocedasticidad y Distribución del Error:** Aunque la transformación Yeo-Johnson corrigió la asimetría severa, los residuos mantuvieron colas ligeramente pesadas (leptocurtosis) explicadas por eventos atípicos extremos no planificados. No obstante, la varianza residual permanece estable a lo largo del horizonte predictivo.
* **Significancia de las Exógenas ($X_t$):** Los p-valores asociados a los coeficientes factoriales ($\gamma_m$) resultaron significativos al 99% de confianza en todas las estaciones, validando la hipótesis de que la integración de la meteorología y contaminantes precursores —libres de colinealidad— mejora el pronóstico frente a series de tiempo aisladas. Únicamente la estación en García resultó apta para predecir por sí misma una gran cantidad de varianza, demostrando autocorrelación dentro del modelo.
...
* **Descripción de gráficas:** El modelo de García resulta ser el más apropiado, ya que demuestra cierta homocedestacidad notable oscilando en el cero, lo que confirma el poco sesgo que tiene este modelo. En el modelo de cadereyta (@fig-residuales-se3), se nota el sesgo con valores atípicos de PM10 en la cola izquierda. Los valores de los residuos indican valores que sobrepasan las 5 y hasta 8 desvaciones estándares a principios de los meses expuestos, posiblemente habiendo un desnivel en esa parte del año. Más allá de ello, la estación de Cadereyta demuestra un comportamiento acertado. Por otro lado, esta la estación de pesquería (@fig-residuales-ne3), comportándose con un ruido de residuos más equilibrados, pero más dispersos. Aún así, la estación de pesquería demuestra la mejor asemejanza a la distribución normal. El resultado que comprueba la presencia de cierta falta de adaptación viable de los hiperparámetros es el de la gráfica ACF que demuestra que los rezagos no lograron incorporar o adaptar adecuadamente a los componentes seleccionados para el modelo.
...
\FloatBarrier
\clearpage
\onecolumn % Pasa a 1 columna completa para que los Anexos respiren y no colapsen
...
https://github.com/INFOAOAA/SIMA_Proyecto