...
def mahalanobis_selection(df: pd.DataFrame):
    cols_to_exclude = [
        'Estacion', 'Station', 'Fecha y hora', 'Fecha', 
        'hora', 'Mes', 'Hora', 'Horas_Registradas'
    ]
    ...
    # CORRECCIÓN 1: Usar pseudo-inversa (pinv) para evitar colapsos por multicolinealidad
...
def main():
    ...
    # CORRECCIÓN 2: Apuntar a la carpeta correcta
    path = os.path.join(
        SCRIPTS_DIR, "..", "Documento", "SIMA_Diario_Limpio.parquet"
    )
